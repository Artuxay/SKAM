-- СКАМ — в чате с ботом сквозным шифрованием шифруются только фото, видео и файлы.
--
-- Текст, стикеры, голосовые и кружочки бот получает как есть, чтобы отвечать на них
-- (как было до e2e_bot_chat). Меняются только фразы бота; права из e2e_bot_chat остаются.
...
begin
  ...
  if new.kind = 'e2e' then
    reply := 'Получил 🔒 Фото, видео и файлы в этом чате зашифрованы сквозным шифрованием — открыть их не могу даже я, ключ есть только у вас.';
